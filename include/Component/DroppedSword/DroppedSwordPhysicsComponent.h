...
class DroppedSwordPhysicsComponent : public PhysicsComponent
{
private:
...
public:
	DroppedSwordPhysicsComponent(bool _isActive, float _acceleration, float _maxSpeed, float _gravityScale,
		float _maxGravity, float _frictionCoefficient, IMessenger* _messenger,
		 IDroppedObjectView* _droppedObjectView);

	DroppedSwordPhysicsComponent(float _acceleration, float _maxSpeed, float _gravityScale, float _maxGravity,
	                             float _frictionCoefficient, IMessenger* _messenger, IDroppedObjectView* _droppedObjectView);

	DroppedSwordPhysicsComponent(bool _isActive, IMessenger* _messenger,
	                             IDroppedObjectView* _droppedObjectView);

	DroppedSwordPhysicsComponent(IMessenger* _messenger, IDroppedObjectView* _droppedObjectView);
